...
struct Field
{
	...
	map<float, vector<Zone*>, greater<float>> aabb_x;
	map<float, vector<Zone*>, greater<float>> aabb_y;
	Field();
	...
	void Clear();
	void Init();
	void AssignToMap(float point, Zone_p& z, map<float, vector<Zone*>, greater<float>>& aabb);
};
